use crate::{create_epub, create_pdf, BookSrc, BookSrcBuilder, EpubOptions, LatexOptions};
...
impl<'a> TryFrom<DeserializableBookSrc<'a>> for BookSrc<'a> {
    ...
    fn try_from(src: DeserializableBookSrc<'a>) -> Result<Self, Self::Error> {
        ...
        for epigraph in src.epigraphs.into_iter() {
            builder.add_explicit_epigraph(
                epigraph.text,
                epigraph.author.as_deref(),
                epigraph.title.as_deref(),
                epigraph.quote_title,
            );
        }
