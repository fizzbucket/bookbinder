...
impl LatexPageStyle {
    ...
    pub(crate) fn default_fancy() -> Self {
        ...
        let mut footer = RunningHeader::default();
        footer.left_even =
            Some(r"\runningtypeface \thepage\footerseperator\currentcontributor".into());
        footer.right_odd = Some(
            r"\runningtypeface \MakeUppercase{\pageidentifier}\footerseperator\thepage".into(),
        );
        LatexPageStyle { header, footer }
    }
}
            ...
            match typeface.into() {
                LatexFont::Named(s) => {
                    if font_exists(&s) {
                        ...
                        eprintln!(
                            "Could not set font '{:?}'; it may not be installed on this system",
                            s
                        );
                        self
                    }
...
impl PreambleOptions {
    ...
    set_font!(
        set_mono_typeface,
        mono,
        doc = "Set the monospace typeface to use in code samples, urls, etc"
    );
    set_font!(
        set_titlepage_typeface,
        titlepage,
        doc = "Set the typeface to use on the titlepage"
    );
    set_font!(
        set_headers_and_footers_typeface,
        running,
        doc = "Set the typeface to use in running headers and footers"
    );
    set_font!(
        set_heading_typeface,
        heading,
        doc = "Set the typeface to use in headings"
    );
    ...
    fn generate_latex_preamble(&self) -> String {
        let fixed_len = PATCHES.len()
            + HOUSEKEEPING.len()
            + PFBREAK_COMMAND.len()
            + NEW_COMMANDS_AND_ENVIRONMENTS.len();
...
impl Default for PreambleOptions {
    fn default() -> Self {
        PreambleOptions {
            ...
            include_toc: false,
            latex_titlesec_options: DEFAULT_TITLESEC_OPTIONS
                .iter()
                .copied()
                .map(|s| s.into())
                .collect(),
