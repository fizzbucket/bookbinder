...
use pulldown_cmark::{LinkType, Alignment};
...
mod quotes;
mod parsing;
pub use parsing::{Parser, InlineParser};
...
impl <'a> From<Event<'a>> for PulldownEvent<'a> {
	fn from(event: Event<'a>) -> Self {
		match event {
			...
			Event::Start(Tag::SmallCaps) => PulldownEvent::Html("<span class=\"smallcaps\">".into()),
			Event::Start(Tag::RightAligned) => PulldownEvent::Html("<span class=\"right-aligned\">".into()),
			Event::Start(Tag::Superscript) => PulldownEvent::Html("<span class=\"superscript\">".into()),
			Event::Start(Tag::Subscript) => PulldownEvent::Html("<span class=\"subscript\">".into()),
	...
	for event in src {
		match event {
			...
			Event::End(Tag::FootnoteDefinition(d)) => {
				...
				if let (Some(Event::Start(Tag::Paragraph)), Some(Event::End(Tag::Paragraph))) = (definition.first(), definition.last()) {
					definition.remove(0);
					definition.pop();
				}
...
‘*Du calme, du calme*.’";
	...
	fn forgotten_closing_quote_does_not_extend_over_para_boundaries() {
		...
		assert_eq!(smart, "‘He's so meticulous\n\nThere was a pause… ‘If you're sure.’");
	}
	...
	fn special_spans() {
		...
		let a = Parser::new(text)
			.collect::<Vec<_>>();
		use Event::*;
		use Tag::*;

		let expected_a = vec![Start(Paragraph), Start(Sans), Text("Hello".into()), End(Sans), Text(" ".into()), Start(SmallCaps), Text("World".into()), End(SmallCaps), End(Paragraph)];
	...
	#[test]
	fn super_and_sub() {		
		let valid_superscripts = Parser::new("'Quoted.' a^bc^d a^hello^")
			.collect::<Vec<_>>();
		let invalid_superscripts = Parser::new("'Quoted.' a^^ a^With space^ unpaired^")
			.collect::<Vec<_>>();
	...
	fn blockquotes() {		
		...
		let text = Parser::new("This checks quotes.\n\n> Single para\n\nNow multi:\n\n> Para 1...\n>\n> Para 2")
			.collect::<Vec<_>>();
		let expected = vec![
			Start(Paragraph),
	...
	fn emphasis_drop() {
		...
		let text = Parser::new("This has *emphasis* (among the 1^st^ of its kind)")
			.collect::<Vec<_>>();
		...
		let expected = vec![
			...
			Text(CowStr::Inlined(InlineStr::try_from("(among the 1").unwrap())),
			Start(Superscript),
			Text(CowStr::Inlined(InlineStr::try_from("st").unwrap())),
			End(Superscript),
			Text(CowStr::Inlined(InlineStr::try_from(" of its kind)").unwrap())),
			End(Paragraph)
		];
