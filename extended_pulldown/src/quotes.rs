...
impl<'a> Replacer<'a> {
    ...
    fn push_to_buffer(&mut self, c: char) {
        match self.str_buffer.as_mut() {
            Some(b) => b.push(c),
            None => self.str_buffer = Some(c.to_string()),
        }
    }
    ...
    fn push_buffer_to_components(&mut self) {
        if let Some(buffer) = std::mem::take(&mut self.str_buffer) {
            self.components
                .push_back(TextComponent::NonQuote(buffer.into()))
        }
    }
    ...
    fn handle_straight_single_quote(&mut self) {
        ...
        match (previous_component, next_char) {
            ...
            (Some(Apostrophe), _) => self.components.push_back(RightQuote),
            ...
            (Some(NonQuote(s)), next) => {
                ...
                match (pc, next) {
                    ...
                    (_, Some(' ')) => self.components.push_back(RightQuote),
                    (_, Some('\n')) => self.components.push_back(RightQuote),
                    (c, Some(d)) if d.is_alphanumeric() => match c {
                        '…' | '—' | '–' => self.components.push_back(LeftQuote),
                        c if c.is_ascii_punctuation() => self.components.push_back(LeftQuote),
                        _ => self.components.push_back(Apostrophe),
                    },
                    (c, Some(d)) if c.is_alphanumeric() => match d {
                        '…' | '—' | '–' => self.components.push_back(RightQuote),
                        c if c.is_ascii_punctuation() => self.components.push_back(RightQuote),
                        _ => self.components.push_back(Apostrophe),
                    },
                    (c, Some(d)) if c.is_ascii_punctuation() => match d {
                        '…' | '—' | '–' => {
                            self.components.push_back(RightQuote);
                        }
...
mod tests {
    ...
    fn single_quotes_text() {
        ...
        let e =
            replace_quotes_ellipsis_and_dashes("'Got a feeling '21 / is going to be a good year'");
        ...
        let j =
            replace_quotes_ellipsis_and_dashes("'Top o' the mornin' to ya,' said the Argentinian.");
        ...
        let n = replace_quotes_ellipsis_and_dashes(
            "an air of whispering, 'Come and find out.'
