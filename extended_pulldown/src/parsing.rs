...
use crate::{Options, Event, Tag, MakeStatic};
...
use pulldown_cmark::{CodeBlockKind, BrokenLink, CowStr, InlineStr};
...
impl <'a, P> Iterator for InlineParser<P> where P: Iterator<Item=Event<'a>> {
	type Item = Event<'a>;
...
impl <'a> Parser<'a> {
	...
	fn convert<'b>(text: Cow<'b, str>) -> CowStr<'b> {
		match text {
			...
			Cow::Owned(s) => {
				match InlineStr::try_from(s.as_str()) {
	...
	pub fn new_with_broken_link_callback(text: &'a str, options: Options, broken_link_callback: Option<&'a mut dyn FnMut(BrokenLink<'_>) -> Option<(CowStr<'a>, CowStr<'a>)>>) -> Self {
		let smarten = options.smarten;
		let inner = PulldownParser::new_with_broken_link_callback(text, options.into(), broken_link_callback);
		Parser{
			inner,
			buffering: Vec::new(),
	...
	fn load_buffer(&mut self) {
				...
				e @ PulldownEvent::Start(PulldownTag::FootnoteDefinition(_)) => {
					...
					while let Some(event) = self.inner.next() {
						match event {
							...
							PulldownEvent::Start(PulldownTag::CodeBlock(CodeBlockKind::Indented)) => {
		...
		for group in text_groups.into_iter() {
			let (indices, text_strs): (Vec<usize>, Vec<CowStr>) = group.into_iter()
				.unzip();

			let text_strs = text_strs.into_iter()
				.map(|s| match s {
			...
			match event {
				...
				PulldownEvent::Html(html) => {
					if html.starts_with("<span") {
						...
						let find_current_span = |style: Option<&str>, class: Option<&str>| -> CurrentSpan {
							if let Some(style) = style {
								 if style.contains("font-family: \"sans\"") || style.contains("font-family:\"sans\"") {
								 	return CurrentSpan::Sans
								 } else if style.contains("font-variant: \"small-caps\"") || style.contains("font-variant:\"small-caps\""){
						...
						match current_span {
							...
							CurrentSpan::SmallCaps => self.buffered.push_back(Event::Start(Tag::SmallCaps)),
							CurrentSpan::Centred => self.buffered.push_back(Event::Start(Tag::Centred)),
							CurrentSpan::RightAligned => self.buffered.push_back(Event::Start(Tag::RightAligned)),
							CurrentSpan::Superscript => self.buffered.push_back(Event::Start(Tag::Superscript)),
							CurrentSpan::Subscript => self.buffered.push_back(Event::Start(Tag::Subscript)),
							CurrentSpan::Generic => {
