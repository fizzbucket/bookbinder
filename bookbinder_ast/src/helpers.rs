...
impl <'a, T: MarkerHelper> CollatedHeader<'a, T> {
	...
	pub fn reconcile_joined_label_and_title(&'a self) -> Option<(Option<Cow<'a, str>>, Option<Cow<'a, str>>)> {
	...
	pub fn get_latex_image_path(&self) -> Result<String, ()> {
		...
		if p.is_latex_supported_image() {
			...
			if let Ok(png) = bookbinder_common::convert_svg_file_to_png(&p, None) {
				let png_path = png.temp_file_path(Some("bookbinder"), "png")
					.to_string_lossy()
					.to_string();
...
impl <'a, I> BookEventIteratorHelper<'a> for I where I: Iterator<Item=BookEvent<'a>> {
	...
	fn collate_division_header<T>(&mut self, is_starred: bool) -> CollatedHeader<'a, T> {
		...
		for event in events.into_iter() {
			match event {
				BookEvent::DivisionHeaderLabel{text, number, number_format} => {
					label_text = text;
					label_number = number;
