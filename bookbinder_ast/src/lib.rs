...
use extended_pulldown::{Parser, flatten_footnotes, MakeStatic, InlineParser};
mod metadata;
...
impl Default for NumberFormat {
	fn default() -> Self {NumberFormat::Arabic}
}
...
macro_rules! read_file_wrapper {
	($doc:meta, $fnname:ident, $wrapped_fn:ident) => {
		#[$doc]
		pub fn $fnname<P, S>(&mut self, filepath: P, title: Option<S>) -> Result<&mut Self, std::io::Error>
		where
			P: AsRef<Path>,
			S: ToString
	...
	($doc:meta, $fnname:ident, $wrapped_fn:ident, $include_authors:expr) => {
		#[$doc]
		pub fn $fnname<A, P>(&mut self, filepath: P, authors: A) -> Result<&mut Self, std::io::Error>
		where
			A: ContributorSource<'a>,
			P: AsRef<Path>
		{
...
impl <'a> BookSrcBuilder<'a> {
	...
	pub fn add_copyright_page<P: ParseHelper<'a>>(&mut self, src: P, title: Option<P>) -> &mut Self {
		...
		if let Some(mut title) = title {
			...
			self.copyright_page.push(BookEvent::BeginSemantic(SemanticRole::Copyrightpage));
			self.copyright_page.push(Event::Start(Tag::UnindentedParagraph).into());
			...
			self.copyright_page.push(Event::End(Tag::UnindentedParagraph).into());
			self.copyright_page.append(&mut events);
			self.copyright_page.push(BookEvent::EndSemantic(SemanticRole::Copyrightpage));
		} else {
	...
	pub fn add_copyright_page_from_file<P: AsRef<Path>>(&mut self, p: P) -> Result<&mut Self, std::io::Error> {
	...
	metadata_func!(paperback_isbn, doc="An isbn for the paperback edition of this book");
	metadata_func!(hardback_isbn, doc="An isbn for the hardback edition of this book");
	...
	metadata_func!(copyright_statement, doc="Set a custom copyright statement");
	...
	metadata_bool_func!(is_not_first_publication, doc="Set this flag if this is not the book's first publication");
	...
	fn read_file_and_add_to_sources<P: AsRef<Path>>(&mut self, path: P) -> Result<String, std::io::Error> {
		...
		if path.is_markdown() {
			...
			Err(std::io::Error::new(std::io::ErrorKind::Other, "Non-markdown file"))
		}
	}
	...
	read_file_wrapper!(doc="Add an appendix from a file", add_appendix_from_file, add_appendix);
	...
	pub fn add_mainmatter_from_file<P: AsRef<Path>>(&mut self, filepath: P) -> Result<&mut Self, std::io::Error> {
		let s = self.read_file_and_add_to_sources(filepath)?;
	...
	pub fn add_explicit_epigraph<P: ParseHelper<'a>>(&mut self, text: P, author: Option<&str>, title_of_source: Option<&str>, quote_title: bool) -> &mut Self {
	...
	fn get_titlepage(&self) -> Vec<BookEvent<'a>> {
		...
		let mut title = self.metadata.get_title()
			.to_string()
			.parse_inline();
		let subtitle = self.metadata.get_subtitle()
	...
	pub fn process(&mut self) -> BookSrc<'a> {
		...
		if !self.no_halftitle && self.halftitle.is_empty() {
			...
			let shorttitle = shorttitle.unwrap_or_else(|| {
		...
		if !self.no_copyrightpage && self.copyright_page.is_empty() {
			let shorttitle = self.metadata.get_short_title()
				.to_string();
		...
		let mut mainmatter = std::mem::take(&mut self.mainmatter)
		...
		if !self.appendices.is_empty() || !self.afterwords.is_empty() || !self.acknowledgements.is_empty() || !self.colophon.is_empty() {
	...
	pub fn change_headers(&mut self, format: TextHeaderOptions) {
		...
		for event in self.contents.iter_mut() {
			match event {
				...
				label @  BookEvent::DivisionHeaderLabel{text: _, number: _, number_format: _} if in_chapter_header => {
					chapter_labels.push(label);
				},
				label @  BookEvent::DivisionHeaderLabel{text: _, number: _, number_format: _} if in_part_header => {
		...
		match (format.chapter_header_format, format.chapter_number_format) {
			(HeaderFormat::NumberAlone, n) => {
				for label in chapter_labels.into_iter() {
					if let BookEvent::DivisionHeaderLabel{text: _, number: _, number_format} = label {
						*number_format = n;
					}
				}
			...
			(HeaderFormat::NumberAndTitle, n) => {
				for label in chapter_labels.into_iter() {
					if let BookEvent::DivisionHeaderLabel{text, number: _, number_format} = label {
						*number_format = n;
						*text = None;
					}
				}
		...
		match (format.part_header_format, format.part_number_format) {
			...
			(HeaderFormat::LabelAlone, n) => {
				for label in part_labels.into_iter() {
					if let BookEvent::DivisionHeaderLabel{text: _, number, number_format} = label {
						*number_format = n;
						*number = None;
					}
				}
...
pub trait ParseHelper<'a>: Sized {
	...
	fn parse_inline(self) -> Vec<BookEvent<'a>> {
		self.parse_inline_plain().into_iter()
			.map(BookEvent::from)
			.collect()
	}
...
pub trait EventHelper<'a> {
	...
	fn remove_initial_title(&mut self) -> Option<Self> where Self: Sized;
...
impl <'a> EventHelper<'a> for Vec<Event<'a>> {
	...
	fn make_uppercase(&mut self) {
		...
		for event in self.iter_mut() {
			match event {
				Event::Text(t) if ignore == 0 => {
					*t = t.to_uppercase().into()
				},
	...
	fn divide_into_sections(self) -> Vec<BookEvent<'a>> {
		...
		let chapter_label: Option<Cow<'static, str>> = SemanticRole::Chapter.get_label()
			.map(|l| l.into());
		let part_label: Option<Cow<'static, str>> = SemanticRole::Part.get_label()
			.map(|l| l.into());

		for event in self.into_iter() {
			match step_level {
				StepLevel::TopToChapter => {
					match event {
						Event::Start(Tag::Heading(1)) => {
							...
							chapter_count += 1;
							collated.push(BookEvent::DivisionHeaderLabel{text: chapter_label.clone(), number: Some(chapter_count), number_format: NumberFormat::Arabic});
						},
				...
				StepLevel::TopToParts => {
					match event {
						Event::Start(Tag::Heading(1)) => {
							...
							collated.push(BookEvent::BeginDivisionHeader(false));
							part_count += 1;
							collated.push(BookEvent::DivisionHeaderLabel{text: part_label.clone(), number: Some(part_count), number_format: NumberFormat::Roman});
...
impl <'a> BookEventHelper for Vec<BookEvent<'a>> {
	...
	fn replace_missing_image_paths(&mut self, image_dirs: &[PathBuf]) -> Result<(), Vec<PathBuf>> {
		...
		for (p, v) in mdests.into_iter().filter(|(k, _v)| !k.exists()) {
			let replacement = image_dirs.iter()
