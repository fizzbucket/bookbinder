...
use crate::svg_titlepage_generator::{WIDTH, HORIZONTAL_MARGIN, TitleEvent, FontData, TypeSetter, SizedText};

const MAX_LINE_WIDTH: i32 = WIDTH - (HORIZONTAL_MARGIN * 2);


fn to_line_items<'a, 'b, I>(text: I, font_data: &FontData<'a, 'b>, italic_font_data: &FontData<'a, 'b>, setter: &TypeSetter<'_>, italic_setter: &TypeSetter<'_>) -> Vec<Item<SizedText<'a>>>
where
		...
		match item {
			TitleEvent::Text(t) => {
				...
				for word in t.split(' ').map(|s| s.to_string()) {
					...
					items.push(Item::Box{width: sized.width, data: sized});
...
pub(crate) fn split_text<'a, 'b, I>(text: I, font_data: &FontData<'a, 'b>, italic_font_data: &FontData<'a, 'b>, setter: &TypeSetter<'_>, italic_setter: &TypeSetter<'_>) -> Vec<Vec<SizedTextOrSpace<'a>>>
where
	I: Iterator<Item=TitleEvent<'a>>
{
	let items = to_line_items(text, font_data, italic_font_data, setter, italic_setter);
	let lengths = std::iter::repeat(700)
		.take(items.len())
		.collect::<Vec<_>>();
	let threshold = 8.0;
	let looseness = 0;
