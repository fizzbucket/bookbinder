...
use bookbinder_ast::helpers::{EpubMarker, BookEventIteratorHelper};
...
use std::borrow::Cow;
use bookbinder_ast::{BookSrc, TextHeaderOptions, SemanticRole, BookEvent};
use extended_pulldown::{Event, Tag, CowStr, CodeBlockKind};
use epub_bundler::{EpubContent, EpubSource, EpubResource, EpubBundlingError};
use uuid::Uuid;
mod svg_titlepage_generator;
mod text_splitting;
use svg_titlepage_generator::{TitleEvent, generate_svg_titlepage};
use std::error::Error;
...
impl Options {
	...
	fn get_titlepage(&self) -> Option<PathBuf> {
		if let Some(ref p) = self.titlepage {
			if p.is_epub_supported_image() { // implies has extension
				if p.is_file() {
					return Some(p.clone())
				}
			} else if p.is_pdf() {
	...
	fn render_to_epub(mut self, mut options: Options) -> Result<Vec<u8>, RenderingError> {
		...
		for page in pages.into_iter() {
			...
			if let Some(toc_title) = page.toc_title {
				content.set_toc_title(toc_title, page.toc_level.unwrap_or(1)).unwrap();
			}
		...
		if let Some(cover_image) = options.cover_image.take() {
			...
			epub_source.set_cover_image(cover_resource)
				.map_err(|_| RenderingError::ImageConversionError(cover_image.clone()))?;
		} else if let Some(titlepage) = options.titlepage.take() {
			let ext = titlepage.extension()
				.map(|s| s.to_str())
				.flatten();
			match ext {
				Some("jpg") => {
...
impl <'a> MetadataAdder for Metadata<'a> {
	fn add_to_epub_source(&self, epub_src: &mut EpubSource) {
		epub_src.set_title(self.title.clone())
			.unwrap();
		...
		add_contributors!(introduction_and_notes_authors, add_author_of_introduction_and_notes);
	}
}
...
impl XhtmlWriter {
	...
	fn write_division_header(&mut self, header: CollatedHeader<'_, EpubMarker>) {
		...
		if let Some((label, title))	= label_and_title {
			...
			if let Some(label) = label {
				self.target.push_str(&format!("<p class=\"division_label\">{}</p>\n", label.to_uppercase()));
			}
	...
	fn get_body(&self) -> String {
		let mut body = format!("<body epub:type=\"{}\">\n\t", get_matter(self.current_division));
	...
	fn get_head(&mut self) -> String {
		...
		let title = self.observed_title.as_deref()
		...
		if get_include_stylesheet(self.current_division) {
			if let Some(css_path) = self.css_path.take() {
				let css_filename = css_path.file_name()
					.unwrap()
					.to_str()
					.unwrap();
		...
		match item {
			...
			Event::End(Tag::Image(_, _, _)) => {
				...
				if self.inline_image_alt_buffer.is_empty() {
					...
					self.target.push_str(&std::mem::take(&mut self.inline_image_alt_buffer));
					self.target.push_str("></img>");
				}
			},
			_ => {}
	...
	fn push_start_tag(&mut self, tag: Tag) {
		...
		match tag {
		    ...
		    FootnoteDefinition(name) => {
		    	...
		    	let number = *self.numbers.entry(name.clone())
		    		.or_insert(len);
			...
			RightAligned => {
				self.target.push_str("<span class=\"align-right float-right\">");
			},
	...
	fn push(&mut self, item: Event<'_>) {
		if self.in_inline_image {
			...
			match item {
				...
				Event::Text(text) => {
					if self.in_heading {
						self.target.push_str(&bookbinder_common::escape_to_html(text.to_uppercase()));
					} else {
				...
				Event::FootnoteReference(name) => {
					let len = self.numbers.len() + 1;
					self.target.push_str(&format!("<a href=\"#{name}\" id=\"fn_ref_{name}\" epub:type=\"noteref\">", name=escape_cowstr_for_html(name.clone())));
		...
		while let Some(event) = events.next() {
			match event {
				...
				BookEvent::BeginTitlePage => {
					...
					let (titlepage_filename, titlepage_filepath) = if let Some(p) = options.get_titlepage() {
						(p.file_name().map(|s| s.to_string_lossy().to_string()).unwrap(), p)
					} else {
						let titlepage_source = TitlePageSource::new(titlepage, options.publisher_imprint_logo.as_deref(), options.titlepage_typeface.as_deref());
