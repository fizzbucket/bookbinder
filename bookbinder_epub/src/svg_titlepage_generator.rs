...
use std::borrow::Cow;
use fontdb::{Style, Family, Weight, Query, Stretch};
use rustybuzz::{UnicodeBuffer, Font};
use ttf_parser::GlyphId;
use std::convert::TryFrom;
use crate::text_splitting::{split_text, SizedTextOrSpace};
use temp_file_name::TempFilePath;
use crate::TitlePageSource;
use bookbinder_common::fonts::{self, LIMITED_FONT_DB, FULL_FONT_DB};
		...
		let (data, face_id) = match family {
			Family::SansSerif => {
				if weight == Weight::BOLD && style == Style::Normal {
					...
					LIMITED_FONT_DB.query(&query)
						.map(|id| LIMITED_FONT_DB.with_face_data(id, |d, i| (Cow::Owned(d.to_vec()), i)))
						.flatten()
						.unwrap()
				}
			},
...
impl <'a> TypeSetter<'a> {
	...
	fn new(data: &'a FontData, size: f32) -> Result<Self, ()> {
		let face = ttf_parser::Face::from_slice(&data.data, data.face_id)
			.map_err(|_| ())?;
		let font = Font::from_slice(&data.data, data.face_id)
			.ok_or(())?;
		
		let units_per_em: f32 = face.units_per_em()
	...
	fn shape(&self, text: &str) -> (i32, i32, i32) {
		...
		let horizontal_len: i32 = positions.iter()
			.map(|p| p.x_advance)
			.sum();
		let (y_mins, y_maxes): (Vec<_>, Vec<_>) = shaped.glyph_infos()
			.iter()
			.map(|g| g.codepoint)
		...
		let units_per_em: i32 = self.face.units_per_em()
			.expect("Error getting units per em")
			.into();
...
impl <'a> SizedText<'a> {
	...
	pub(crate) fn new<S: Into<Cow<'a, str>>>(data: &FontData<'a, '_>, setter: &TypeSetter, text: S) -> Self {
		let text = text.into();
...
impl SVGWriter {
	...
	fn add_divider(&mut self) {
		...
		self.svg.push_str(&format!("<line x1=\"{}\" x2=\"{}\" y1=\"{}\" y2=\"{}\" stroke=\"black\"/>\n", x1, x2, self.y, self.y));
		self.y += RULE_SPACING;
	}
	...
	fn add_logo(&mut self, logo: PathBuf) {
		match image::open(&logo) {
			Ok(img) => {
				...
				let img = img.resize(max_image_width as u32, MAX_LOGO_HEIGHT, FilterType::Gaussian);
				let (width, height) = img.dimensions();
				let mut new_image = Vec::new();
				img.write_to(&mut new_image, ImageOutputFormat::Png).unwrap();
			...
			Err(e) => {
				match bookbinder_common::convert_to_jpg(&logo) {
					Err(_) => eprintln!("could not render image for titlepage logo ({:?}): {}", logo.display(), e),
					Ok(jpg) => {
...
fn generate_titlepage_svg<'a, S, I, P>(title: I, subtitle: Option<I>, contributors: Option<Vec<(Option<&'a str>, Vec<S>)>>, logo: Option<P>, typeface: Option<&'a str>) -> Result<String, ()>
where
	...
	PathBuf: From<P>
{
	...
	let title_lines = split_text(title.into_iter(), &bold_font_data, &bold_italic_font_data, &title_setter, &title_emph_setter);
	let subtitle_lines = subtitle.map(|subtitle| split_text(subtitle.into_iter(), &normal_font_data, &italic_font_data, &subtitle_setter, &subtitle_emph_setter));
	...
	if let Some(contributors) = contributors {
		...
		for (role, names) in contributors.iter() {
			...
			if let Some(role) = role {
				let sized_role = SizedText::new(&normal_font_data, &contributor_ancillary_setter, role.to_uppercase());
			...
			match names.len() {
				...
				_ => {
					...
					let and = SizedText::new(&normal_font_data, &contributor_ancillary_setter, "AND");
