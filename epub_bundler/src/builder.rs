...
use crate::{
    Contributor, EpubContent, EpubResource, EpubSource, Identifier, Title,
    TitleCode
};
use epub_metadata::{
    DublinCoreElement, EpubTitleType, MarcRelator, OnixContributorCode, OnixProductIdentifier,
    OnixTitleCode, ValueMapping, ContributorRole
};
...
impl <'a> NavInfo<'a> {
	fn render(&self) -> String {
		let mut out = String::from(r#"<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops">"#);
		out.push_str("\n\t<head>\n");
		out.push_str(&format!("\t\t<title>{}</title>\n", self.toc_title));
		out.push_str("\t\t<meta charset=\"utf-8\"></meta>\n");
		if let Some(ref stylesheet) = self.stylesheet {
			out.push_str(&format!("\t\t<link rel=\"stylesheet\" href=\"{}\" type=\"text/css\"></link>\n", stylesheet));
		}
		out.push_str("\t</head>\n");
		out.push_str("\t<body>\n");
...
pub(crate) trait EpubBundler {
    ...
    fn get_spine_items<'a, 'b>(manifest: &'a [ManifestItem<'a>]) -> Result<Vec<SpineElement<'b>>, EpubBundlingError>;
...
impl EpubBundler for EpubSource {
    ...
    fn get_nav(&self) -> Result<String, EpubBundlingError> {
        ...
        let tocs = self.contents.iter()
            .filter_map(|x| x.toc_entry.as_ref()
                .map(|e| (x.output_path.as_path(), e))
            );
...
impl TEManager {
    ...
    fn add_new_entry<S: ToString, P: AsRef<Path>>(&mut self, href: P, level: usize, text: S) {
        ...
        let parent_idx = self.entries.iter()
            .rposition(|x| x.get_level() < level);
...
impl TE {
    ...
    fn with_child(&self, child_idx: usize) -> Self {
        match self {
            TE::Branch{idx, level, parent, href, text, children} => {
            ...
            TE::Leaf{idx, level, parent, href, text} => {
                TE::Branch {
                    idx: *idx,
                    level: *level,
                    parent: *parent,
...
mod tests {
    ...
    fn test_opf() -> Result<(), Box<dyn Error>> {
        ...
        let opf= opf.split("\n")
        	.filter(|line| {
        		if line.trim().starts_with("<meta property=\"dcterms:modified\" id=\"last_modification\"") {
        			false
        		} else {
        			true
        		}
    ...
    fn test_toc_stuff() {
        ...
        assert_eq!(
            toc_entries.get_top_level_entries()
                .into_iter()
                .map(|e| match e {
                    TE::Branch{text, ..} => text,
                    TE::Leaf{text, ..} => text
